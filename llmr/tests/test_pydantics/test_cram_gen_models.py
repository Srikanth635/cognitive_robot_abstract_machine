...
from llmr.workflows.pydantics.cram_gen_models import (
    Cutting,
    Holding,
    Mixing,
    Peeling,
...
class TestHoldingModel:
    def test_construction(self) -> None:
        h = Holding(holder="robot", held_object="cup", action_verb="hold")
        assert h.holder == "robot"
        assert h.held_object == "cup"
        assert h.duration is None


class TestPeelingModel:
    def test_construction(self) -> None:
        p = Peeling(agent="robot", object="apple", action_verb="peel")
        assert p.agent == "robot"
        assert p.object == "apple"
