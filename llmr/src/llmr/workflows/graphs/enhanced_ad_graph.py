...
from ..agents.intent_entity import ReflectiveParser
from ..lg_memory.memory.extractor import MemoryExtractor
from ..lg_memory.memory.long_term_store import LongTermMemoryStore
from ..lg_memory.memory.retriever import MemoryRetriever
...
from ..pydantics.cram_gen_models import (
    Adding,
    Arranging,
    Baking,
    Cooling,
...
memory_db = MongoDBSaver(
    ...
    allowed_msgpack_modules=[
        ("llmr.workflows.pydantics.intent_entity_models", "IntentType"),
        ("llmr.workflows.pydantics.intent_entity_models", "StatusType"),
        ("llmr.workflows.pydantics.intent_entity_models", "PriorityType"),
    ],
)
...
def run_with_cache(
    ...
    """Run the graph, optionally using semantic caching.

    Args:
        instruction: Natural language instruction.
        user_id: User identifier for memory scoping.
        thread_id: Thread identifier for checkpointing.
        use_cache: When True (default) a cached result is returned on hit and
                   the result is stored after a miss.  Set to False to always
                   run the full LLM pipeline and skip cache read/write.
    """
    if use_cache:
