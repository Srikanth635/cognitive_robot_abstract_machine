"""
memory/long_term_store.py
Long-term memory — stores facts, preferences, summaries across threads.
Persisted in MongoDB. You control what gets saved and when.
"""
...
class LongTermMemoryStore:
    """
    Manages long-term user memories in MongoDB.

    Each document represents one memory unit (a fact, preference, summary, or entity).
    Documents can optionally carry an embedding vector for semantic search.
    """
    ...
    def semantic_search(
        ...
        """Semantic similarity search using MongoDB Atlas Vector Search.

        Prerequisites:
        - Running on MongoDB Atlas (not local)
        - Vector index named 'memory_vector_index' created on 'embedding' field
        """
