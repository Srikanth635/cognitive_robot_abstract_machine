"""
memory/retriever.py
Retrieves relevant long-term memories to inject into prompts.
Combines tag-based and semantic search for best results.
"""
...
class MemoryRetriever:
    ...
    Strategy:
    1. Always load recent facts + preferences (cheap, always useful)
    2. If embedder available -> semantic search on the current message
    ...
    def retrieve(
        ...
        """Return a de-duplicated list of memory dicts.

        Priority order:
        1. Semantic matches on current message (if embedder available)
        2. Tag matches (if tags provided)
        3. Recent facts and preferences as fallback
        """
