"""
memory/extractor.py
Uses the LLM to intelligently extract memories from conversations.
This is what makes long-term memory *smart* rather than rule-based.
"""
...
from .long_term_store import LongTermMemoryStore, MemoryType
...
class MemoryExtractor:
    """Analyzes conversations and extracts structured memories using an LLM.

    Extracted memories are saved to the LongTermMemoryStore.
    """
...
"""
    ...
    def __init__(
        ...
        self.store = store
        self._llm = None  # lazy — instantiated on first use
    ...
    def generate_summary(
        ...
        """Generate a conversation summary and save it as a 'summary' memory.

        Returns the MongoDB document ID, or None if the conversation is too short.
        """
