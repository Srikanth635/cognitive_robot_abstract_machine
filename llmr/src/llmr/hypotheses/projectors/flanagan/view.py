...
class FlanaganGraphView(ReasonerGraphView):
    ...
    ROOT_CLAIM_TYPES = (MotionPlanHypothesisNode,)
    CLAIM_TYPES = (MotionPlanHypothesisNode, MotionPhaseHypothesisNode)

    def plans(self) -> list[MotionPlanHypothesisNode]:
    ...
    ) -> list[MotionPhaseHypothesisNode]:
    ...
    def phases_for_plan(
        self, plan_node: MotionPlanHypothesisNode
    ) -> list[MotionPhaseHypothesisNode]:
        """Return phase claims attached to *plan_node* through HasMotionPhaseEdge."""

        return [
            phase
            for phase in self.graph.get_targets(
                plan_node.id, HasMotionPhaseEdge, MotionPhaseHypothesisNode
            )
            if phase.meta.source_reasoner == self.REASONER_NAME
        ]

    def contact_phases(self) -> list[MotionPhaseHypothesisNode]:
        """Return motion phases that expect contact."""

        return [phase for phase in self.phases() if phase.contact]

    def phases_with_failures(self) -> list[MotionPhaseHypothesisNode]:
        """Return motion phases that include explicit failure modes."""

        return [phase for phase in self.phases() if phase.possible_failures]

    def high_urgency_phases(self) -> list[MotionPhaseHypothesisNode]:
