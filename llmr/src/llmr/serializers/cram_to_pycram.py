"""CRAM-to-PyCRAM Serializer
============================

Converts the LISP-style CRAM plan strings produced by the llmr
`enhanced_ad_graph` workflow into PyCRAM ``ActionDescription`` dataclass
instances (or ``PartialDesignator`` wrappers ready for execution).

Architecture
------------
Three distinct layers keep concerns separated:

1. **Parser** (``CRAMParser``) — tokenises and recursively parses a CRAM
   S-expression string into a nested Python list tree, then extracts a flat
   dict of semantic roles (``action_type``, ``object``, ``source``, ``goal``,
   ``utensil``, ``count``, ``technique``).

2. **Intermediate representation** (``CRAMActionPlan``) — a plain Python
   dataclass holding only symbolic string data extracted from the CRAM plan.
   Completely decoupled from PyCRAM.

3. **Mapper** (``CRAMToPyCRAMMapper``) — converts a ``CRAMActionPlan`` into
   a PyCRAM ``PartialDesignator`` by calling user-supplied resolver callables
   that bind symbolic entity names/types to live PyCRAM ``Body`` objects.

The top-level ``CRAMToPyCRAMSerializer`` class wires all three layers
together as a convenience API.

Supported CRAM → PyCRAM action mappings
-----------------------------------------
CRAM action_type          PyCRAM class
------------------------  --------------------------------
PickingUp / picking_up    PickUpAction            (core)
Placing  / placing        PlaceAction             (core)
Opening  / open-object    OpenAction              (core)
Closing  / shutting       CloseAction             (core)
Lifting  / lift-object    PickUpAction            (core, best match)
Cutting  / cut-object     CuttingAction           (composite)
Mixing   / mixing         MixingAction            (composite)
Pouring  / pouring        PouringAction           (composite)
Transporting              TransportAction         (composite)
Navigating                NavigateAction          (core)
Detecting                 DetectAction            (core)
...
::
    ...
    serializer = CRAMToPyCRAMSerializer()

    # --- Step 1: parse the CRAM string to an intermediate plan ----
    cram_str = (
    ...
    plan = serializer.parse(cram_str)
    print(plan.action_type)      # 'PickingUp'
    print(plan.object.tag)       # 'cup'
    print(plan.source.tag)       # 'table'

    # --- Step 2: map to a PyCRAM PartialDesignator ----------------
    #   Provide a body_resolver that maps symbolic entity info → Body objects.
    #   You can also supply default arm / grasp_description here.

    def my_resolver(entity_info):  # CRAMEntityInfo → Body
        return world.get_body_by_name(entity_info.tag or entity_info.semantic_type)

    from pycram.datastructures.enums import Arms
    partial = serializer.to_partial_designator(
        plan,
        body_resolver=my_resolver,
        arm=Arms.RIGHT,
    )
    partial.perform()   # execute on the robot

    # --- Or use the one-shot convenience method -------------------
    partial = serializer.serialize(cram_str, body_resolver=my_resolver)
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# Section 7 – Standalone demo  (python -m  or  python cram_to_pycram.py)
# ─────────────────────────────────────────────────────────────────────────────

if __name__ == "__main__":

    # claude --resume 9e3e456c-5758-4658-bbe2-8690f9050984
    """Standalone demo — no PyCRAM installation required.

    Run with:
        cd cognitive_robot_abstract_machine
        python -m src.serializers.cram_to_pycram

    or directly:
        python src/serializers/cram_to_pycram.py
    """
    ...
    def _show_plan(plan: "CRAMActionPlan") -> None:
        """Print a CRAMActionPlan in a readable table format."""
        rows = [
            ("action_type",    plan.action_type),
            ("object.tag",     plan.object.tag),
            ("object.type",    plan.object.semantic_type),
            ("object.props",   plan.object.properties or "—"),
            ("source.tag",     plan.source.tag or "—"),
            ("source.type",    plan.source.semantic_type or "—"),
            ("source.relation",plan.source.relation or "—"),
            ("goal.tag",       plan.goal.tag or "—"),
            ("goal.type",      plan.goal.semantic_type or "—"),
            ("goal.relation",  plan.goal.relation or "—"),
            ("utensil.tag",    plan.utensil.tag or "—"),
            ("utensil.type",   plan.utensil.semantic_type or "—"),
            ("amount",         plan.amount if plan.amount is not None else "—"),
            ("unit",           plan.unit or "—"),
            ("technique",      plan.technique or "—"),
        ]
        width = max(len(k) for k, _ in rows)
        for key, val in rows:
            print(f"  {key:<{width}}  {val}")

    # ── mock body resolver (simulates a PyCRAM world look-up) ──────────────

    class _MockBody:
        """Minimal stand-in for a PyCRAM Body object."""
        def __init__(self, name: str, sem_type: str = ""):
            self.name = name
            self.sem_type = sem_type
        def __repr__(self) -> str:
            return f"MockBody(name={self.name!r}, type={self.sem_type!r})"

    def mock_body_resolver(entity: "CRAMEntityInfo") -> "_MockBody":
        """Return a MockBody from symbolic entity info.

        In a real PyCRAM session you would replace this with something like:
            lambda e: world.get_body_by_name(e.tag or e.semantic_type)
        """
        if not entity:
            return None
        name = entity.tag or entity.semantic_type or "unknown"
        return _MockBody(name=name, sem_type=entity.semantic_type or "")

    # ── mock PyCRAM action stubs (used when pycram is not installed) ────────

    class _MockPartialDesignator:
        """Stand-in for pycram.datastructures.partial_designator.PartialDesignator."""
        def __init__(self, action_cls_name: str, **kwargs):
            self.action_cls_name = action_cls_name
            self.kwargs = kwargs
        def __repr__(self) -> str:
            kw = ", ".join(f"{k}={v!r}" for k, v in self.kwargs.items())
            return f"PartialDesignator[{self.action_cls_name}]({kw})"
        def perform(self):
            print(f"  [MOCK] Performing {self.action_cls_name} with {self.kwargs}")

    def _make_mock_action_cls(name: str):
        """Dynamically create a mock PyCRAM action class with a .description() factory."""
        def description(cls, **kwargs):
            return _MockPartialDesignator(name, **kwargs)
        description = classmethod(description)
        return type(name, (), {"description": description})

    # ── patch mapper to use mock classes when pycram is absent ─────────────

    import importlib as _importlib

    _original_load = CRAMToPyCRAMMapper._load_pycram_class

    def _patched_load(self, class_name: str):
        try:
            return _original_load(self, class_name)
        except (ImportError, ModuleNotFoundError):
            return _make_mock_action_cls(class_name)

    CRAMToPyCRAMMapper._load_pycram_class = _patched_load

    # ── also patch Arms default when pycram absent ─────────────────────────

    try:
        from pycram.datastructures.enums import Arms as _Arms
        DEFAULT_ARM = _Arms.RIGHT
    except (ImportError, ModuleNotFoundError):
        DEFAULT_ARM = "Arms.RIGHT (mock)"

    # ══════════════════════════════════════════════════════════════════════════
    # Example CRAM plans (realistic outputs from enhanced_ad_graph)
    # ══════════════════════════════════════════════════════════════════════════

    EXAMPLES = [
        # ── 1. PickingUp ────────────────────────────────────────────────────
        # Actual output from dynamic_actions.json — source is nested inside object
        (
            "PickingUp — cup from table (nested source style)",
            "(an action (type PickingUp) "
            "(object (:tag cup (an object (type Artifact size medium color white "
            "material ceramic shape cylindrical condition whole handle present))) "
            "(source (a location (on (:tag table (an object (type Surface size large "
            "material wood shape rectangular position on)))))))",
        ),

        # # ── 2. Cutting ──────────────────────────────────────────────────────
        # # Standard sibling roles + count
        # (
        #     "Cutting — apple with knife, 4 pieces",
        #     "(an action (type cutting) "
        #     "(object (:tag apple (an object (type apple) (color red) (is-edible true)))) "
        #     "(count (unit pieces) (number 4)) "
        #     "(utensil (:tag knife (an object (type knife) (material stainless-steel)))))",
        # ),
        #
        # # ── 3. Placing ──────────────────────────────────────────────────────
        # (
        #     "Placing — cup on counter",
        #     "(an action (type Placing) "
        #     "(object (:tag cup (an object (type cup.n.01) (material ceramic)))) "
        #     "(target (a location (on (:tag counter (an object (type countertop.n.01) "
        #     "(material granite)))))))",
        # ),
        #
        # # ── 4. Mixing ───────────────────────────────────────────────────────
        # (
        #     "Mixing — batter in bowl with whisk",
        #     "(an action (type mixing) "
        #     "(object (:tag bowl (an object (type mixing_bowl.n.01) (size large)))) "
        #     "(utensil (:tag whisk (an object (type whisk.n.01) (material stainless-steel)))) "
        #     "(technique spiral_mixing))",
        # ),
        #
        # # ── 5. Pouring ──────────────────────────────────────────────────────
        # (
        #     "Pouring — milk from jug into bowl, 200 ml",
        #     "(an action (type pour) "
        #     "(source (:tag milk_jug (an object (type jug.n.01) (contains milk)))) "
        #     "(destination (a location (inside (:tag bowl (an object (type bowl.n.01)))))) "
        #     "(count (unit ml) (number 200)))",
        # ),
        #
        # # ── 6. Opening ──────────────────────────────────────────────────────
        # (
        #     "Opening — fridge door",
        #     "(perform (an action (type open-object) "
        #     "(object (:tag fridge_door (an object (type door.n.01) "
        #     "(part-of fridge) (direction outward))))))",
        # ),
        #
        # # ── 7. Transporting ─────────────────────────────────────────────────
        # (
        #     "Transporting — plate to dining table",
        #     "(an action (type transporting) "
        #     "(object (:tag plate (an object (type plate.n.01) (color white) (size medium)))) "
        #     "(goal (a location (on (:tag dining_table "
        #     "(an object (type table.n.01) (location dining_room)))))))",
        # ),
        #
        # # ── 8. Lifting ──────────────────────────────────────────────────────
        # # (maps to PickUpAction — best available core action)
        # (
        #     "Lifting — pot (maps → PickUpAction)",
        #     "(perform (an action (type lift-object) "
        #     "(object (:tag pot (an object (type pot.n.01) (material cast_iron) (weight heavy))))))",
        # ),
    ]
    ...
    print(f"\n{'═'*68}")
    print("  CRAM → PyCRAM Serializer  —  Standalone Demo")
    print(f"{'═'*68}")

    for idx, (label, cram_str) in enumerate(EXAMPLES, start=1):

        _hdr(f"Example {idx}: {label}")

        # ── Step 1: show the raw CRAM string ──────────────────────────────
        print("\n  [INPUT] CRAM plan string:")
        # word-wrap the string at ~62 chars for readability
        words, line = [], ""
        for tok in cram_str.split():
            if len(line) + len(tok) > 62:
                print(f"    {line}")
                line = tok
            else:
        ...
        # ── Step 2: parse to intermediate representation ──────────────────
        print(f"\n  [PARSE] → CRAMActionPlan")
        try:
            plan = ser.parse(cram_str)
            _show_plan(plan)
        except ValueError as exc:
            print(f"  ERROR during parse: {exc}")
            continue

        # ── Step 3: show which PyCRAM class this maps to ──────────────────
        mapper = ser._mapper
        key = mapper._normalise_action_type(plan.action_type)
        pycram_cls_name = mapper._ACTION_MAP.get(key, f"UNKNOWN (normalised key: '{key}')")
        print(f"\n  [MAP]   CRAM '{plan.action_type}' → PyCRAM '{pycram_cls_name}'")

        # ── Step 4: create the PartialDesignator via the mock resolver ────
        print(f"\n  [SERIALIZE] → PartialDesignator (using MockBody resolver)")
        try:
            partial = ser.to_partial_designator(
                plan,
                body_resolver=mock_body_resolver,
                arm=DEFAULT_ARM,
            )
            print(f"    {partial}")
    ...
    # # ── Batch demo ─────────────────────────────────────────────────────────
    # _hdr("Batch: serialize_many on all examples at once")
    #
    # cram_strings = [cram for _, cram in EXAMPLES]
    # try:
    #     partials = ser.serialize_many(
    #         cram_strings,
    #         body_resolver=mock_body_resolver,
    #         arm=DEFAULT_ARM,
    #     )
    ...
    #
    # # ── Debug utilities ────────────────────────────────────────────────────
    # _hdr("Debug utilities: parse_raw() and extract_roles_from()")
    #
    # demo_cram = EXAMPLES[1][1]  # Cutting example
    # print(f"\n  Input: {demo_cram[:60]}...")
    #
    # print("\n  parse_raw() → nested list tree:")
    # raw = CRAMToPyCRAMSerializer.parse_raw(demo_cram)
    # import json as _json
    # print(_json.dumps(raw, indent=4))
    #
    # print("\n  extract_roles_from() → flat roles dict:")
    # roles = CRAMToPyCRAMSerializer.extract_roles_from(demo_cram)
