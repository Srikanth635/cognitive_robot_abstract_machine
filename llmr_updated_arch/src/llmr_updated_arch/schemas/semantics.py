...
Three categories:
  ...
  - :class:`ActionAnnotationBundle` (+ :class:`FlanaganMotionPlan`,
    ...
    PyCRAM execution — populated by pluggable :class:`~llmr_updated_arch.generation.Reasoner`
...
class MotionPhase(BaseModel):
    ...
    :class:`~llmr_updated_arch.generation.flanagan_reasoner.FlanaganReasoner`.
...
class FlanaganMotionPlan(BaseModel):
    ...
    Produced by :class:`~llmr_updated_arch.generation.flanagan_reasoner.FlanaganReasoner`
...
class FrameNetAnnotation(BaseModel):
    ...
    Produced by :class:`~llmr_updated_arch.generation.framenet_reasoner.FrameNetReasoner` and
...
class ActionAnnotationBundle(BaseModel):
    ...
    by :class:`~llmr_updated_arch.generation.Reasoner` implementations after slot filling
    ...
    reasoners that need it (e.g. :class:`~llmr_updated_arch.generation.framenet_reasoner.FrameNetReasoner`)
    can read it without receiving the backend as a dependency."""

    # ── Core reasoners ────────────────────────────────────────────────────────
    classification: Optional[ActionClassificationResult] = None
    """Output of the action-classification step (:func:`~llmr_updated_arch.generation.slot_filler.classify_action`)."""

    slot_filling: Optional[SlotFillingOutput] = None
    """Output of the slot-filling step (:func:`~llmr_updated_arch.generation.slot_filler.run_slot_filler`)."""
    ...
    :class:`~llmr_updated_arch.generation.framenet_reasoner.FrameNetReasoner`."""
...
class SemanticBundle(ActionAnnotationBundle):
    """General semantic artifact store used by the component pipeline.

    The typed fields inherited from :class:`ActionAnnotationBundle` preserve the
    current llmr behavior for slot filling, FrameNet, and Flanagan outputs. The
    ``artifacts`` bag lets new generators attach structured outputs without
    changing the bundle schema first.
    """

    action_type: str = ""
    artifacts: Dict[str, Dict[str, Any]] = Field(default_factory=dict)

    def put_artifact(self, generator_name: str, artifact_type: str, value: Any) -> None:
        self.artifacts.setdefault(generator_name, {})[artifact_type] = value

    def get_artifact(self, generator_name: str, artifact_type: str) -> Any:
        return self.artifacts.get(generator_name, {}).get(artifact_type)
