...
from llmr_updated_arch.integrations.krrood.introspect import FieldKind, ActionFieldIntrospector
from llmr_updated_arch.core.errors import LLMActionRegistryEmpty
from llmr_updated_arch.schemas import ActionClassificationResult, SlotFillingOutput

if TYPE_CHECKING:
    from langchain_core.language_models import BaseChatModel
    from llmr_updated_arch.integrations.krrood.introspect import ActionSpec as ActionSchema, DiscoveredField as FieldSpec
...
class SlotSemanticsGenerator:
    """Pipeline component that produces the slot-filling semantic artifact."""

    name = "slot_semantics"

    def generate(self, context: Any) -> Any:
        """Run slot filling for the free slots in *context*.

        The returned artifact is intentionally still semantic: entity slots hold
        descriptions, not grounded SymbolGraph objects.
        """
        from llmr_updated_arch.core.contracts import SemanticArtifact

        match_data = context.match_snapshot
