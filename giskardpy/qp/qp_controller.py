            ...
            if df.shape[1] > 1:
                for column_name, column in df.T.items():
                    zero_filtered_column = (
                        column.replace(0, np.nan).dropna(how="all").replace(np.nan, 0)
                    )
...
class QPController:
    ...
    def init(
        ...
        self.degrees_of_freedoms = list(
            sorted(degrees_of_freedom, key=lambda dof: str(dof.name))
        )
        self.dof_filter = np.array(
            [
                god_map.world.state.keys().index(dof.name)
                for dof in self.degrees_of_freedoms
            ]
        )
    ...
    def _create_debug_pandas(self) -> None:
        ...
        num_vel_constr = len(self.qp_adapter.derivative_constraints) * (
            self.config.prediction_horizon - 2
        )
        num_eq_vel_constr = len(self.qp_adapter.eq_derivative_constraints) * (
            self.config.prediction_horizon - 2
        )
        ...
        self.p_weights = pd.DataFrame(
            qp_data.quadratic_weights, free_variable_names, ["data"], dtype=float
        )
        self.p_g = pd.DataFrame(
            qp_data.linear_weights, free_variable_names, ["data"], dtype=float
        )
        self.p_lb = pd.DataFrame(
            qp_data.box_lower_constraints, free_variable_names, ["data"], dtype=float
        )
        self.p_ub = pd.DataFrame(
            qp_data.box_upper_constraints, free_variable_names, ["data"], dtype=float
        )
        ...
        if len(qp_data.eq_bounds) > 0:
            self.p_bE_raw = pd.DataFrame(
                qp_data.eq_bounds, equality_constr_names, ["data"], dtype=float
            )
            self.p_bE = deepcopy(self.p_bE_raw)
            self.p_bE[
                len(self.qp_adapter.equality_bounds.names_derivative_links) :
            ] /= self.config.mpc_dt
        else:
            self.p_bE = pd.DataFrame()
        if len(qp_data.neq_lower_bounds) > 0:
            self.p_lbA_raw = pd.DataFrame(
                qp_data.neq_lower_bounds, inequality_constr_names, ["data"], dtype=float
            )
            self.p_lbA = deepcopy(self.p_lbA_raw)
            self.p_lbA /= self.config.mpc_dt

            self.p_ubA_raw = pd.DataFrame(
                qp_data.neq_upper_bounds, inequality_constr_names, ["data"], dtype=float
            )
            self.p_ubA = deepcopy(self.p_ubA_raw)
            self.p_ubA /= self.config.mpc_dt

            self.p_bA_raw = pd.DataFrame(
                {"lbA": qp_data.neq_lower_bounds, "ubA": qp_data.neq_upper_bounds},
                inequality_constr_names,
                dtype=float,
            )
        ...
        if self.xdot_full is not None:
            ...
            self.p_xdot = pd.DataFrame(
                xdot_full, free_variable_names, ["data"], dtype=float
            )
            ...
            if len(self.p_A) > 0:
                ...
                bA_slack = self.p_xdot[
                    len(self.p_xdot) - num_neq_constr - num_vel_constr :
                ]
