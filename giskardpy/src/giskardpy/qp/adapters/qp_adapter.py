...
class ProblemDataPart(ABC):
    ...
    def _remove_columns_columns_where_variables_are_zero(
        self, free_variable_model: cas.Matrix, max_derivative: Derivatives
    ) -> cas.Matrix:
    ...
    def velocity_limit(
        ...
        if self.config.prediction_horizon == 1:
            return cas.Vector([lower_limits.velocity]), cas.Vector(
...
class Weights(ProblemDataPart):
    ...
    def construct_expression(
        ...
        if linear_weights is None:
            ...
            linear_weights = cas.Matrix(linear_weights)
            linear_weights = cas.vstack(
                [linear_weights]
                + [cas.Scalar(0)] * (weights.shape[0] - linear_weights.shape[0])
            )
        return cas.Vector(weights), linear_weights
    ...
    def derivative_weight_expressions(self) -> List[Dict[str, cas.Scalar]]:
        params = []
    ...
    def eq_derivative_weight_expressions(self) -> List[Dict[str, cas.Scalar]]:
        params = []
        for d in Derivatives.range(Derivatives.velocity, self.config.max_derivative):
            derivative_constr_weights = {}
            for t in range(self.config.prediction_horizon):
...
class FreeVariableBounds(ProblemDataPart):
    ...
    def derivative_slack_limits(
        self, derivative: Derivatives
    ) -> Tuple[Dict[str, cas.Scalar], Dict[str, cas.Scalar]]:
        lower_slack = {}
        upper_slack = {}
        for t in range(self.config.prediction_horizon):
    ...
    def construct_expression(
        ...
        return cas.Vector(lb), cas.Vector(ub)
...
class EqualityBounds(ProblemDataPart):
    ...
    def equality_constraint_bounds(self) -> Dict[str, cas.Scalar]:
        return {
...
class InequalityBounds(ProblemDataPart):
    ...
    def derivative_constraint_bounds(
        self, derivative: Derivatives
    ) -> Tuple[Dict[str, cas.Vector], Dict[str, cas.Vector]]:
        lower = {}
        upper = {}
        for t in range(self.config.prediction_horizon):
            for c in self.get_derivative_constraints(derivative):
    ...
    def implicit_pos_model_limits(
        self,
    ) -> Tuple[List[Dict[str, cas.Vector]], List[Dict[str, cas.Vector]]]:
        lb_acc, ub_acc = {}, {}
        lb_jerk, ub_jerk = {}, {}
        for v in self.degrees_of_freedom:
    ...
    def construct_expression(
        ...
        lb_params: List[Dict[str, cas.Vector]] = []
        ub_params: List[Dict[str, cas.Vector]] = []
        ...
        return cas.Vector(lbA), cas.Vector(ubA)
...
class EqualityModel(ProblemDataPart):
    ...
    def equality_constraint_expressions(self) -> List[cas.Matrix]:
        return self._sorter(
    ...
    def velocity_constraint_model(self) -> Tuple[cas.Matrix, cas.Matrix]:
        """
        model
        ...
        if number_of_vel_rows > 0:
            ...
            for derivative in Derivatives.range(
                ...
                eye = cas.Matrix.eye(self.config.prediction_horizon)[
            ...
            slack_model = cas.Matrix.eye(num_slack_variables) * self.config.mpc_dt
    ...
    @profile
    def derivative_link_model(self, max_derivative: Derivatives) -> cas.Matrix:
        """
        ...
        derivative_link_model = cas.Matrix.zeros(num_rows, num_columns)

        x_n = cas.Matrix.eye(num_rows)
        derivative_link_model[:, : x_n.shape[0]] += x_n

        xd_n = -cas.Matrix.eye(num_rows) * self.config.mpc_dt
    ...
    @profile
    def derivative_link_model_no_acc(self, max_derivative: Derivatives) -> cas.Matrix:
        """
        Layout for prediction horizon 5
        Slots are matrices of |controlled variables| x |controlled variables|
        ...
        model = cas.Matrix.zeros(rows=n_jerk, columns=n_jerk + n_vel)
        pre_previous = -cas.Matrix.eye(n_vel)
        same = pre_previous
        previous = -2 * pre_previous
        j_same = cas.Matrix.eye(n_jerk) * self.config.mpc_dt**2
    ...
    def _remove_rows_columns_where_variables_are_zero(
        self, derivative_link_model: cas.Matrix
    ) -> cas.Matrix:
    ...
    @profile
    def equality_constraint_model(self) -> Tuple[cas.Matrix, cas.Matrix]:
        """
        ...
        if (
            ...
            if len(self.constraint_collection.eq_constraints) > 0:
                model = cas.Matrix.zeros(
                    len(self.constraint_collection.eq_constraints),
                    self.number_of_non_slack_columns,
                )
                J_eq = (
                    cas.Matrix(self.equality_constraint_expressions()).jacobian(
    ...
    def construct_expression(
        ...
        max_derivative = Derivatives.velocity
        derivative_link_model = cas.Matrix()
        ...
        if len(model_parts) == 0:
            ...
            model = cas.Matrix(np.empty((0, self.number_of_non_slack_columns)))
        else:
        ...
        slack_model = cas.vstack(
            [
                cas.Matrix.zeros(derivative_link_model.shape[0], slack_model.shape[1]),
                slack_model,
            ]
        )
...
class InequalityModel(ProblemDataPart):
    ...
    def inequality_constraint_expressions(self) -> List[cas.Matrix]:
        return self._sorter(
    ...
    def acceleration_constraint_model(self) -> Tuple[cas.Matrix, cas.Matrix]:
        """
        ...
        if number_of_acc_rows > 0:
            ...
            J_vel_block = cas.Matrix.eye(self.config.prediction_horizon).kron(Jd_q)
            J_acc_block = cas.Matrix.eye(self.config.prediction_horizon).kron(
                J_q + Jd_qd
            )
            J_jerk_block = cas.Matrix.eye(self.config.prediction_horizon).kron(J_qd)
            horizontal_offset = (
                self.number_of_free_variables * self.config.prediction_horizon
            )
    ...
    def inequality_constraint_model(
        ...
        if (
            ...
            if len(self.constraint_collection.neq_constraints) > 0:
                ...
                J_neq = (
                    cas.Matrix(self.inequality_constraint_expressions()).jacobian(
                        variables=self.get_free_variable_symbols(Derivatives.position),
                    )
                    * self.config.mpc_dt
                )
    ...
    def implicit_pos_limits(self) -> Tuple[cas.Matrix, cas.Matrix]:
        """
        pk = pk-1 + vk*dt
            p0 = pc + v0*dt
            pk/dt - pk-1/dt = vk
        ...
        """
        n_vel = self.number_of_free_variables * (self.config.prediction_horizon - 2)
        model = cas.Matrix.tri(n_vel) * self.config.mpc_dt
        slack_model = cas.Matrix.zeros(model.shape[0], self.number_ineq_slack_variables)
        return model, slack_model
