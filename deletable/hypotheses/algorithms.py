...
from llmr.hypotheses.projectors.framenet.nodes import RoleClaimNode
...
def invalidate_from_symbol(
    ...
    *,
    ...
    Walks every ancestor of each GroundingEvidenceNode bound to
...
def conflicting_role_claims(
    graph: HypothesisGraph,
    action_ref: Any,
) -> List[Tuple[RoleClaimNode, RoleClaimNode]]:
    """Find RoleClaimNode pairs with the same role name but different fillers.
    ...
    roles = subgraph.domain(RoleClaimNode)

    by_role: Dict[str, List[RoleClaimNode]] = {}
    for role in roles:
        by_role.setdefault(role.role_name, []).append(role)

    conflicts: List[Tuple[RoleClaimNode, RoleClaimNode]] = []
