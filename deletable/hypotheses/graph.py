...
class HypothesisGraph:
    ...
    _symbol_groundings_by_symbol_ref_id: Dict[
        int, List[GroundingEvidenceNode]
    ...
    def add_node(self, node: TNode) -> TNode:
        ...
        if isinstance(node, GraphLinked) and not hasattr(node, "_graph_ref"):
            node._graph_ref = weakref.ref(self)  # type: ignore[attr-defined]
        return node
    ...
    def __iter__(self):
        ...
        Enables ``variable(NodeType, domain=graph)`` via EQL's is_iterable
        path — EQL filters the yielded nodes with isinstance internally.
        """
    ...
    def _index_special(self, node: HypothesisNode) -> None:
        if isinstance(node, GroundingEvidenceNode):
