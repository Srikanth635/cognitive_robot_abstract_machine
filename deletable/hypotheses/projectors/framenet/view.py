...
class FrameNetGraphView(ReasonerGraphView):
    ...
    ROOT_CLAIM_TYPES = (FrameClaimNode,)
    CLAIM_TYPES = (FrameClaimNode, RoleClaimNode)

    def frames(self) -> list[FrameClaimNode]:
        """Return FrameNet frame claims preserving graph insertion order."""

        return [
            node
            for node in self.graph.domain(FrameClaimNode)
    ...
    def roles(self) -> list[RoleClaimNode]:
        ...
        return [
            node
            for node in self.graph.domain(RoleClaimNode)
            if node.meta.source_reasoner == self.REASONER_NAME
        ]

    def frames_by_frame(self, frame_name: str) -> list[FrameClaimNode]:
    ...
    def roles_by_role_name(self, role_name: str) -> list[RoleClaimNode]:
    ...
    def roles_for_frame(
        self, frame_node: FrameClaimNode
    ) -> list[RoleClaimNode]:
        ...
        return [
            ...
            for role in self.graph.get_targets(
                frame_node.id, HasRoleEdge, RoleClaimNode
            )
            if role.meta.source_reasoner == self.REASONER_NAME
        ]

    def grounded_roles(self) -> list[RoleClaimNode]:
    ...
    def supported_roles(self) -> list[RoleClaimNode]:
    ...
    def hypothesis_only_roles(self) -> list[RoleClaimNode]:
