...
def add_textured_mesh_to_glb_scene(
    textured_mesh_dict,
    scene=None,
    ...
    if preserve_coordinate_system:
        ...
        vertices_transformed[:, 2] = -vertices_transformed[
            :, 2
        ]  # Flip new Z to maintain handedness
    ...
    if faces.max() >= len(vertices):
        raise ValueError(
            f"Face indices exceed vertex count: max face index {faces.max()}, vertex count {len(vertices)}"
        )

    # Validate that texture face indices are valid for texture coordinates
    if fts.max() >= len(vts):
        raise ValueError(
            f"Texture face indices exceed texture coordinate count: max fts index {fts.max()}, vts count {len(vts)}"
        )
    ...
    vertex_buffer_view = BufferView(
        buffer=buffer_index, byteOffset=0, byteLength=vertex_size, target=ARRAY_BUFFER
    )
    ...
    material = Material(name=material_name, pbrMetallicRoughness=pbr_metallic_roughness)
    ...
    primitive = Primitive(
        attributes=Attributes(
            POSITION=vertex_accessor_index, TEXCOORD_0=texcoord_accessor_index
        ),
    ...
    if hasattr(scene, "_buffer_data") and scene._buffer_data:
        ...
        for i, buffer_data in scene._buffer_data.items():
            if i < len(scene.buffers):
                ...
                buffer_info.append(
                    (i, total_size, len(buffer_data), aligned_size, buffer_data)
                )
        ...
        for buffer_idx, offset, original_size, aligned_size, buffer_data in buffer_info:
            ...
            if aligned_size > original_size:
                unified_buffer[offset + original_size : offset + aligned_size] = (
                    b"\x00" * (aligned_size - original_size)
                )
...
def load_glb_to_mesh_dict(glb_path):
    ...
    position_offset = position_buffer_view.byteOffset + (
        position_accessor.byteOffset or 0
    )
    position_data = binary_blob[
        position_offset : position_offset + position_buffer_view.byteLength
    ]
    ...
    texcoord_offset = texcoord_buffer_view.byteOffset + (
        texcoord_accessor.byteOffset or 0
    )
    texcoord_data = binary_blob[
        texcoord_offset : texcoord_offset + texcoord_buffer_view.byteLength
    ]
    ...
    indices_data = binary_blob[
        indices_offset : indices_offset + indices_buffer_view.byteLength
    ]
    ...
    if image.uri and image.uri.startswith("data:"):
        ...
        image_bytes = binary_blob[
            image_offset : image_offset + image_buffer_view.byteLength
        ]
    else:
