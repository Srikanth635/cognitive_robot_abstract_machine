"""Phase 1 LangGraph agent: NL instruction → typed per-action slot schema.

    START → slot_filler_node → END

The agent makes a single LLM call that:
...
all supported actions.  After the LLM call the agent converts this into the
...
from ..schemas.pick_up import EntityDescriptionSchema, GraspParamsSchema, PickUpSlotSchema
from ..schemas.place import PlaceSlotSchema, TargetLocationDescriptionSchema
...
class _SlotFillerOutput(BaseModel):
    """Private schema used only inside this agent for the LLM structured output call.
    ...
    agent — all public functions return ``PickUpSlotSchema`` or ``PlaceSlotSchema``.
    """
    ...
    # PlaceAction only
    target_description: Optional[TargetLocationDescriptionSchema] = Field(
        default=None,
...
def run_slot_filler(
    ...
    """Run the slot-filler agent and return a typed per-action schema.
