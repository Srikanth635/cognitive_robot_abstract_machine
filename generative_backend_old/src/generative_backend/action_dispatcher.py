...
from .workflows.prompts.pick_up import pick_up_resolver_prompt
from .workflows.prompts.place import place_resolver_prompt
...
from .workflows.schemas.place import (
    PlaceDiscreteResolutionSchema,
    PlaceSlotSchema,
    TargetLocationDescriptionSchema,
)
...
class ActionHandler(ABC):
    ...
    @staticmethod
    def _target_to_entity(desc: TargetLocationDescriptionSchema) -> EntityDescriptionSchema:
        return EntityDescriptionSchema(
            name=desc.name,
            semantic_type=desc.semantic_type,
            spatial_context=desc.spatial_context,
...
class PickUpActionHandler(ActionHandler):
    ...
    # Lazy-initialised resolver LLM
    _resolver_llm = None

    @classmethod
    def _get_resolver_llm(cls):
    ...
    def execute(self, schema: PickUpSlotSchema) -> PickUpAction:
        ...
        logger.info("PickUp grounding – %d body/bodies (tier=%s).", len(grounding.bodies), grounding.tier)

        # Step 2 — build PartialDesignator
        partial = self._build_partial(schema, grounding.bodies)
        print("Partial Desginator :", partial.to_dict())
        ...
        print("LLM reasoning for missing params :", resolution.reasoning)
    ...
    def _parse_grasp(
        ...
        if params is None:
            return None
        if params.approach_direction is None or params.vertical_alignment is None or params.rotate_gripper is None:
    ...
    def _resolve_discrete(
        ...
        print("World Context for pickup action : \n",world_ctx)
    ...
    def _build_world_context(self, partial: PartialDesignator[PickUpAction]) -> str:
        ...
        for obj in objects:
            ...
            try:
                ann_types = [type(a).__name__ for a in self._world.get_semantic_annotations_of_body(obj)]
                if ann_types:
...
class PlaceActionHandler(ActionHandler):
    ...
    _resolver_llm = None  # lazy-initialised
    ...
    def execute(self, schema: PlaceSlotSchema) -> PlaceAction:
        ...
        tgt_grounding = self._ground(self._target_to_entity(schema.target_description))
        ...
        logger.info(
            "Place grounding – object: %d body/bodies (tier=%s), target: %d body/bodies (tier=%s).",
            len(obj_grounding.bodies), obj_grounding.tier,
            len(tgt_grounding.bodies), tgt_grounding.tier,
        )
        ...
        if partial.missing_parameter():
            ...
            print("LLM reasoning for place : ", resolution.reasoning)

        return PlaceAction(object_designator=obj_body, arm=arm, target_location=tgt_body)

    def _resolve_arm(self, obj_bodies: list, tgt_bodies: list) -> PlaceDiscreteResolutionSchema:
        world_ctx = self._build_world_context(obj_bodies, tgt_bodies)
        print("world context for place : ", world_ctx)
        chain = place_resolver_prompt | self._get_resolver_llm()
        try:
            return chain.invoke({
                "world_context": world_ctx,
                "known_parameters": "None – arm not specified.",
                "parameters_to_resolve": "arm  (choose LEFT or RIGHT based on "
                                         "which arm holds the object and target position)",
            })
        except Exception as exc:
            raise RuntimeError(f"Place discrete resolver LLM failed: {exc}") from exc

    def _build_world_context(self, obj_bodies: list, tgt_bodies: list) -> str:
        lines = []
        robot_xyz = None
        ...
        for obj in obj_bodies:
            ...
            try:
                ...
                lines.append(
                    ...
                    if xyz else f"Object to place '{name}': pose unknown"
                )
            except Exception:
        ...
        for tgt in tgt_bodies:
            ...
            try:
                ...
                if tgt_xyz:
                    ...
                    if robot_xyz is not None:
                        ...
                        side = "right" if dy < 0 else "left"
                        front_back = "in front of" if dx > 0 else "behind"
                        lines.append(f"  → {abs(dx):.2f}m {front_back} and {abs(dy):.2f}m to the {side} of the robot.")
                else:
            ...
            try:
                types = [type(a).__name__ for a in self._world.get_semantic_annotations_of_body(tgt)]
                if types:
