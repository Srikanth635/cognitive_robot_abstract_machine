...
from .workflows.nodes.slot_filler import ActionSlotSchema, run_slot_filler
...
def _serialise_world_for_llm(world: World) -> str:
    ...
    try:
        ...
        all_ann_types: list[str] = []
        for b in list(world.bodies)[:20]:
            anns = world.get_semantic_annotations_of_body(b)
            if anns:
                b_name = str(getattr(getattr(b, "name", None), "name", b))
                type_names = [type(a).__name__ for a in anns]
                ann_summary[b_name] = type_names
                all_ann_types.extend(type_names)
        # Deduplicated list — used by the slot filler to constrain semantic_type
        unique_types = sorted(set(all_ann_types))
        if unique_types:
            lines.append(f"\nAvailable semantic types: {', '.join(unique_types)}")
        if ann_summary:
            lines.append("\nSemantic annotations per body:")
