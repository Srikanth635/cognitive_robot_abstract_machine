"""Phase 2 LangGraph node: world context + partial params → discrete resolution schema.

Graph structure:
    START → resolver_node → END

A single generic node handles discrete parameter resolution for *any* action
type.  The prompt template and output schema class are injected at call time,
so adding a new action type requires no changes here — only a new prompt and
schema in the appropriate ``prompts/`` and ``schemas/`` files.

The typed convenience wrappers (``run_pickup_resolver``, ``run_place_resolver``)
provide a clean, type-safe API for callers that know the action type up front.
...
Usage::
    ...
    # PickUp — resolves arm, approach_direction, vertical_alignment, rotate_gripper
    result = run_pickup_resolver(
    ...
    # Place — resolves arm only
    result = run_place_resolver(
        world_context="...",
        known_parameters="None – arm not specified.",
        parameters_to_resolve="arm  (choose LEFT or RIGHT ...)",
    )
"""
...
from ..prompts.place import place_resolver_prompt
from ..schemas.pick_up import PickUpDiscreteResolutionSchema
from ..schemas.place import PlaceDiscreteResolutionSchema
