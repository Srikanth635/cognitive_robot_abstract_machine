...
class TestForwardReferenceResolution:
    ...
    def test_multiple_forward_references_resolution(self):
        ...
        modules_to_remove = [
            ...
            if 'isolated_forward_ref' in key
        ]
        for mod in modules_to_remove:
            del sys.modules[mod]
        
        # Import IsolatedTypeAlpha so it's in sys.modules and can be found
        # But IsolatedTypeBeta is NOT imported, so it won't be found
        from test.krrood_test.dataset.isolated_forward_ref_types import IsolatedTypeAlpha
