...
def test_semantic_as_distribution(immutable_model_world):
    ...
    costmap = AlgebraicSemanticCostmap(world.get_body_by_name("island_countertop"))
    costmap.valid_area = (
        costmap.valid_area.__deepcopy__() & costmap.right() & costmap.bottom() & costmap.border(0.2)
    )
...
def test_semantic_iterate(immutable_model_world):
    ...
    costmap = AlgebraicSemanticCostmap(world.get_body_by_name("island_countertop"))
    costmap.valid_area = (
        costmap.valid_area.__deepcopy__() & costmap.left() & costmap.bottom() & costmap.border(0.2)
    )
...
def test_probabilistic_visibility_cm(immutable_model_world):
    world, robot_view, context = immutable_model_world
    origin = PoseStamped.from_list([1.5, 1, 0], [0, 0, 0, 1], world.root)
    costmap = ProbabilisticCostmap(origin, size=200 * centimeter, costmap_type=VisibilityCostmap)
    costmap.visualize()
...
def test_probabilistic_merge_cm(immutable_model_world):
    world, robot_view, context = immutable_model_world
    origin = PoseStamped.from_list([1.5, 1, 0], [0, 0, 0, 1], world.root)
    visibility = ProbabilisticCostmap(origin, size=200 * centimeter, costmap_type=VisibilityCostmap)
