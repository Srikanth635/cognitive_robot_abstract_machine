"""Motion precondition planner: computes navigation/body-pose actions before task actions.

For each task action (PickUpAction, PlaceAction, ...) this module figures out
what preparatory motions the robot needs:

  - Navigate to a reachable base pose (via CostmapLocation).
  - Move the torso to the correct height (HIGH before reaching, LOW after).
  - Park arms (before navigation).

The planner is registry-based: each action type registers a ``PreconditionProvider``
subclass.  Adding a new action type requires only:

  1. Implementing ``YourPreconditionProvider(PreconditionProvider)``.
  2. Calling ``MotionPreconditionPlanner.register(YourAction, YourPreconditionProvider)``.

The ``ExecutionLoop`` and ``MotionPreconditionPlanner`` core require zero changes.
...
class ExecutionState:
    ...
    Providers read this state when they need information from previous actions
    (e.g. PlaceAction needs to know which arm was used by the preceding PickUpAction).

    This state is also serialised into the LLM world-context string so the
    slot-filler and resolver know the current robot arm occupancy when planning
    each instruction.
    """
...
class PreconditionResult:
    ...
    :param preconditions: Ordered list of actions to perform *before* the task action.
    :param action: The task action, possibly with updated fields (e.g. a resolved
                   ``target_location`` PoseStamped for PlaceAction).
    """
...
class PickUpPreconditionProvider(PreconditionProvider):
    """Prepares the robot to pick up an object.

    Precondition sequence:
      1. ``ParkArmsAction``         – tuck only the arms not currently holding objects.
      2. ``MoveTorsoAction(HIGH)``  – raise torso for better arm reach.
      3. ``NavigateAction(pose)``   – move base to a reachable position.
    """
    ...
    def _ensure_free_arm(action: PickUpAction, exec_state: ExecutionState) -> PickUpAction:
        ...
        logger.info(
            ...
            arm.name,
            getattr(getattr(exec_state.held_objects[arm], "name", None), "name",
...
class PlacePreconditionProvider(PreconditionProvider):
    """Prepares the robot to place a held object on a surface.

    Precondition sequence:
      1. ``MoveTorsoAction(HIGH)``  – raise torso to reach placement height.
      2. ``NavigateAction(pose)``   – move base to a position reachable for placing.

    Additionally this provider:
      - Resolves ``action.target_location`` from a ``Body`` to a concrete
        ``PoseStamped`` via ``SemanticCostmapLocation`` (surface + object height).
      - Overrides ``action.arm`` by looking up which arm is holding the object in
        ``exec_state.held_objects``, so multi-pickup sequences use the correct arm.
    """
...
class MotionPreconditionPlanner:
    ...
    :class:`ExecutionLoop` uses this to compute the full action sequence for each
    task action emitted by the LLM pipeline.

    Registering a new action type::

        MotionPreconditionPlanner.register(MyAction, MyPreconditionProvider)
