...
returns them as an ordered list of atomic sub-instructions together with a dependency
graph.  Dependencies are based on OBJECT FLOW — a PlaceAction that places an object
acquired by a prior PickUpAction depends on that PickUp, regardless of whether they
are adjacent in the list.  Atomic instructions are returned unchanged (single-element
...
Usage::
    ...
    plan = decomposer.decompose("fetch the milk and place it on the island_countertop")
    # plan.steps       → ["Pick up the milk", "Place the milk on the island_countertop"]
    # plan.dependencies → {1: [0]}

    plan = decomposer.decompose("pick up the milk")
    # plan.steps       → ["pick up the milk"]
    # plan.dependencies → {}
"""
...
class DecomposedPlan:
    """Result of decomposing a (possibly compound) NL instruction.

    :param steps: Ordered list of atomic sub-instructions, each mapping to exactly
        one supported action type.
    :param dependencies: Mapping from step index to the list of step indices that
        must succeed before this step can execute.  Keys with empty lists or absent
        keys represent independent steps.  Indices are 0-based within ``steps``.

    Example — "fetch the milk and place it on the counter"::

        DecomposedPlan(
            steps=["Pick up the milk", "Place the milk on the counter"],
            dependencies={1: [0]},
        )

    Example — "pick milk. pick bowl. place milk"::

        DecomposedPlan(
            steps=["Pick up the milk", "Pick up the bowl", "Place the milk on the counter"],
            dependencies={2: [0]},   # bowl pickup is independent
        )
    """
...
class TaskDecomposer:
    ...
    def decompose(self, instruction: str) -> DecomposedPlan:
        ...
        try:
            ...
            _PICKUP_VERBS = {"pick", "grab", "grasp", "lift", "take", "fetch", "get", "retrieve"}
            _PLACE_VERBS  = {"place", "put", "set", "deposit", "lay", "drop"}
            ...
            if n_verbs == 1 and len(steps) > 1:
                ...
                logger.warning(
                    ...
                    len(steps), instruction, steps[best],
                )
            ...
            logger.info(
                ...
                instruction, steps, dependencies,
            )
