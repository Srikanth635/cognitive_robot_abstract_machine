...
## Pipeline flow
    ...
    NL instruction
        │
        ▼  slot_filler node  (classify + extract in one LLM call)
...
3. Update ``../workflows/nodes/slot_filler.py`` to classify and return the new schema.
...
if TYPE_CHECKING:
    from ..planning.motion_precondition_planner import ExecutionState
...
def _serialise_robot_state(exec_state: "ExecutionState") -> str:
    ...
    if exec_state.held_objects:
        ...
        body_name = str(getattr(getattr(body, "name", None), "name", body)) if body else "unknown object"
